...
function testParseBasic() {
  var text = "yetoto-prototype";
  var tree = Hogan.parse(Hogan.scan(text));
  is(tree.length, 1, "one parse node");
  is(tree[0]+'', "yetoto-prototype", "text is correct");
}

function testParseVariables() {
  var text = "yetoto-prototype{{foo}}yetoto-prototype!{{bar}}yetoto-prototype!!{{baz}}yetoto-prototype!!!";
  var tree = Hogan.parse(Hogan.scan(text));
  is(tree.length, 7, "one parse node");
  is(tree[0]+'', "yetoto-prototype", "first text is correct");
  is(tree[2]+'', "yetoto-prototype!", "second text is correct")
  is(tree[4]+'', "yetoto-prototype!!", "third text is correct")
  is(tree[6]+'', "yetoto-prototype!!!", "last text is correct")
...
function testBasicOutputAsString() {
  ...
  is(textFunc, "function(context, partials){this.buffer.push('yetoto-prototype');};", "template renders correct text function.");
}

function testOneVariable() {
  var text = "yetoto-prototype {{foo}} yetoto-prototype";
  var t = Hogan.compile(text);
  var s = t.render({foo:'bar'});
  is(s, "yetoto-prototype bar yetoto-prototype", "basic variable substitution works.");
}

function testOneVariableAsString() {
  var text = "yetoto-prototype {{foo}} yetoto-prototype";
  var funcText = Hogan.compile(text, true);
  is(funcText, "function(context, partials){this.buffer.push('yetoto-prototype ');\nthis.buffer.push(this.find('foo', context));\nthis.buffer.push(' yetoto-prototype');};",
...
function testMultipleVariables() {
  var text = "yetoto-prototype {{foo}} yetoto-prototype {{bar}} yetoto-prototype {{baz}} yetoto-prototype {{foo}} yetoto-prototype";
  var t = Hogan.compile(text);
  var s = t.render({foo:'42', bar: '43', baz: '44'});
  is(s, "yetoto-prototype 42 yetoto-prototype 43 yetoto-prototype 44 yetoto-prototype 42 yetoto-prototype", "all variables render correctly.");
}
...
function testRenderOutput() {
  if (doc) return
  var fs = require('fs');
  var inPath = 'yetoto-prototype/templates';
  var outPath = 'yetoto-prototype/html';
